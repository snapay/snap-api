...
function bootstrapView () {
  ...
  try {
    ...
    getInitialJson(function (response) {
      ...
      $('.publish-block').length && (fetchCustomDomains(), bindPublishPageHandlers());
...
function attachHandlers () {
  ...
  $('.is-expandable').on('click', showInModal);
...
function attachSidebarHandlers () {
  ...
  $(window).on('activate.bs.scrollspy', function (e) {
    var activeElement = $('.nav-link[href="' + e.target.hash + '"]');
    scrollIntoView(activeElement);
    autoToggleFolderState(activeElement);
...
function autoToggleFolderState ($activeElement) {
  var $currentFolder = $activeElement.closest('.folder'),
    toc,
    request;
  ...
  if ($currentFolder.length === 0) {
    ...
    // is it a toc or a request?
    toc = $activeElement.closest('.toc');
    request = $activeElement.closest('.request');

    if (toc.length !== 0) {
      $currentFolder = toc;
    } else {
      $currentFolder = request;
    }
  } else {
    // open folder if not already
    !$currentFolder.hasClass('open') && openFolder($currentFolder);
  }

  // close the previous folder if it was not opened manually
  if (activeFolder && $currentFolder.get(0) != activeFolder.get(0)) {
    activeFolder.is('.folder.open') && !activeFolder.hasClass('user-opened') &&
      activeFolder.removeClass('open')
        .find('ul')
...
function scrollIntoView ($activeElement) {
  var sidebarEl = $('.sidebar').get(0),
    navbarOffset = $('#navbar-example').offset().top,
    sidebarIsScrollable = sidebarEl.scrollHeight > sidebarEl.clientHeight,
    activeElementOffset = $activeElement.offset().top;

  if (sidebarIsScrollable) {
    $('.sidebar').scrollTop(activeElementOffset - navbarOffset);
  }
}
...
function highlightParentFolder ($activeElement) {
  var isFolder = $activeElement.parent().hasClass('folder-link'),
    ...
    $currentFolderLink = $currentFolder.find('.folder-link a');
 ...
 */
function toggleDomainHelpTextVisibility (label) {
  var helptext = {
      unverified: $('#custom-domain-message__is-unverified'),
      unavailable: $('#custom-domain-message__is-unavailable')
    };

  _.isString(label) && _.forEach(helptext, function ($el, tag) {
    label.match(new RegExp('\\(' + tag + '\\)$')) && $el.show() || $el.hide();
  });
};
...
function bindPublishPageHandlers () {
  $('body')
    .on('click', '.publish-section-custom-domain li', function () {
      ...
      toggleDomainHelpTextVisibility(label);
    })
